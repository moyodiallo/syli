...
let report_type_error (loc : Syli_typing.Typed_ast.location option)
    (msg : string) =
  match loc with
  | None -> Printf.eprintf "Type error: %s\n" msg
  | Some loc ->
      Syli_parsing.Error_reporting.show_error ~kind:"Type"
        ~filename:loc.filename ~start_pos:loc.start_pos ~end_pos:loc.end_pos
        ~msg;
      exit 1

let build (filename : string) : unit =
  ...
  in
  let find_tool name =
    if Sys.command ("command -v " ^ name ^ " >/dev/null 2>&1") = 0 then name
    else
      let candidates =
  ...
  let opt_cmd =
    ...
    Printf.sprintf
      "%s -passes='default<O3>,rewrite-statepoints-for-gc' -S %s -o %s" opt
      ll_file rewritten_file
  in
  ...
  let link_flags =
    if os = "Darwin" then
      ...
      Printf.sprintf
        "%s -O3 -fno-pie -no-pie -Wno-override-module -Wl,-T,%s -o %s %s %s %s \
         -lm"
  ...
  if Array.length Sys.argv < 3 then usage ()
    ...
    try
      (match command with
        ...
        | _ -> usage ())
      |> ignore
    with Syli_typing.Env.Type_error (loc, msg) -> report_type_error loc msg
