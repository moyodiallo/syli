...
val unify :
  ?loc:Typed_ast.location -> Subst.t -> Typed_ast.ty -> Typed_ast.ty -> Subst.t
(** Unifies two types, threading and extending the substitution. Raises
    [Type_error] if unification fails; the error is annotated with [?loc] when
    provided. *)
...
val unify_into :
  ?loc:Typed_ast.location ->
  Env.infer_ctx ->
  Typed_ast.ty ->
  Typed_ast.ty ->
  Env.infer_ctx
(** Unifies two types within an inference context. Raises [Type_error] on
    failure, annotated with [?loc] when provided. *)
