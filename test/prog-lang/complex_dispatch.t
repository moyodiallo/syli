Complex test combining closures, dispatch, casts, partial application, and if-then-else:
  $ cat >complex_dispatch.sy <<EOF
  > foreign syli_print_i64 : i64 -> unit = "syli_print_i64"
  > let add x y z = x
  > let apply f x y = f x y
  > let main () =
  >   let add1 = add 1
  >   let r1 = apply add1 10 20
  >   syli_print_i64 r1
  >   let r2 = apply add1 1.0 2.0
  >   syli_print_i64 r2
  >   let add1and2 = add1 2
  >   let r3 = add1and2 30
  >   syli_print_i64 r3
  >   let r4 = add1and2 3.0
  >   syli_print_i64 r4
  >   let r5 =
  >     if true then
  >       apply add1 100 200
  >     else
  >       apply add1 1.0 2.0
  >   syli_print_i64 r5
  > EOF

  $ dune exec sylic -- core complex_dispatch.sy > complex_dispatch.core
  $ dune exec sylic -- cir complex_dispatch.sy > complex_dispatch.cir
  $ dune exec sylic -- oir complex_dispatch.sy > complex_dispatch.oir
  $ dune exec sylic -- llvm complex_dispatch.sy > complex_dispatch.ll
  $ cat complex_dispatch.core
  module Complex_dispatch
  extern syliComplex_dispatch.syli_print_i64 : (i64) -> unit
  
  let syliComplex_dispatch.add = fun (x, y, z) : 'a148 ->
      x : 'a148
  
  let syliComplex_dispatch.apply = fun (f, x, y) : 'a163 ->
      f(x : 'a157, y : 'a159) : 'a163
  
  let syliComplex_dispatch.main = fun () : unit ->
      {
        let sy1_add1 = syliComplex_dispatch.add(1 : i64) : ('a167) -> ('a168) -> i64
        let sy2_r1 = syliComplex_dispatch.apply(sy1_add1 : (i64) -> (i64) -> i64, 10 : i64, 20 : i64) : i64
        syliComplex_dispatch.syli_print_i64(sy2_r1 : i64) : unit
        let sy3_r2 = syliComplex_dispatch.apply(sy1_add1 : (f64) -> (f64) -> i64, 1.0 : f64, 2.0 : f64) : i64
        syliComplex_dispatch.syli_print_i64(sy3_r2 : i64) : unit
        let sy4_add1and2 = sy1_add1(2 : i64) : ('a183) -> i64
        let sy5_r3 = sy4_add1and2(30 : i64) : i64
        syliComplex_dispatch.syli_print_i64(sy5_r3 : i64) : unit
        let sy6_r4 = sy4_add1and2(3.0 : f64) : i64
        syliComplex_dispatch.syli_print_i64(sy6_r4 : i64) : unit
        let sy7_r5 = if true : bool
            syliComplex_dispatch.apply(sy1_add1 : (i64) -> (i64) -> i64, 100 : i64, 200 : i64) : i64
          else
            syliComplex_dispatch.apply(sy1_add1 : (f64) -> (f64) -> i64, 1.0 : f64, 2.0 : f64) : i64
        syliComplex_dispatch.syli_print_i64(sy7_r5 : i64) : unit
      }
  
  $ cat complex_dispatch.cir
  module Complex_dispatch :
  ffi_external_functions:
  extern fn syli_print_i64(i64) -> void
  
  
  functions:
  public fn __init.Complex_dispatch() -> void:
    entry: bb0
  
    bb0:
  
      return
  end
  
  public fn syliComplex_dispatch.main(%__unit.0:i64) -> void:
    entry: bb0
  
    bb0:
      %Sy_cir_var_0:(?167, ?168 -> i64) = #make_closure {syliComplex_dispatch.add} () ( captured_args=[1:i64])
      %Sy_cir_var_1:(i64, i64 -> i64) = cast(%Sy_cir_var_0:(?167, ?168 -> i64) as (i64, i64 -> i64))
      %Sy_cir_var_2:i64 = #call_direct syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64 (%Sy_cir_var_1:(i64, i64 -> i64), 10:i64, 20:i64)
      %Sy_cir_var_3:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_2:i64)
      %Sy_cir_var_4:(f64, f64 -> i64) = cast(%Sy_cir_var_0:(?167, ?168 -> i64) as (f64, f64 -> i64))
      %Sy_cir_var_5:i64 = #call_direct syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64 (%Sy_cir_var_4:(f64, f64 -> i64), 1.0f:f64, 2.0f:f64)
      %Sy_cir_var_6:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_5:i64)
      %Sy_cir_var_7:(?183 -> i64) = #partial_apply {%Sy_cir_var_0:(?167, ?168 -> i64)} (2:i64)
      %Sy_cir_var_8:i64 = #call_apply {%Sy_cir_var_7:(?183 -> i64) as (i64 -> i64)}  (30:i64)
      %Sy_cir_var_9:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_8:i64)
      %Sy_cir_var_10:i64 = #call_apply {%Sy_cir_var_7:(?183 -> i64) as (f64 -> i64)}  (3.0f:f64)
      %Sy_cir_var_11:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_10:i64)
      %Sy_cir_var_12:bool = cast(true:bool as bool)
      cond_br %Sy_cir_var_12:bool, bb1, bb2
  
    bb2:
      %Sy_cir_var_16:(f64, f64 -> i64) = cast(%Sy_cir_var_0:(?167, ?168 -> i64) as (f64, f64 -> i64))
      %Sy_cir_var_17:i64 = #call_direct syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64 (%Sy_cir_var_16:(f64, f64 -> i64), 1.0f:f64, 2.0f:f64)
      %Sy_cir_var_13:i64 = move(%Sy_cir_var_17:i64)
      goto bb3
  
    bb1:
      %Sy_cir_var_14:(i64, i64 -> i64) = cast(%Sy_cir_var_0:(?167, ?168 -> i64) as (i64, i64 -> i64))
      %Sy_cir_var_15:i64 = #call_direct syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64 (%Sy_cir_var_14:(i64, i64 -> i64), 100:i64, 200:i64)
      %Sy_cir_var_13:i64 = move(%Sy_cir_var_15:i64)
      goto bb3
  
    bb3:
      %Sy_cir_var_18:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_13:i64)
      return
  end
  
  public fn syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64(%f:(f64, f64 -> i64), %x:f64, %y:f64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_cir_var_0:i64 = #call_apply {%f:(f64, f64 -> i64)}  (%x:f64, %y:f64)
      return %Sy_cir_var_0:i64
  end
  
  public fn syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64(%f:(i64, i64 -> i64), %x:i64, %y:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_cir_var_0:i64 = #call_apply {%f:(i64, i64 -> i64)}  (%x:i64, %y:i64)
      return %Sy_cir_var_0:i64
  end
  
  public fn syliComplex_dispatch.add__i64__i64__f64_ret_i64(%x:i64, %y:i64, %z:f64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  public fn syliComplex_dispatch.add__i64__i64__i64_ret_i64(%x:i64, %y:i64, %z:i64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  public fn syliComplex_dispatch.add__i64__f64__f64_ret_i64(%x:i64, %y:f64, %z:f64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  end

  $ cat complex_dispatch.oir
  module Complex_dispatch :
  ffi_external_functions:
  extern fn syli_print_i64(i64) -> void
  
  
  functions:
  public fn __init.Complex_dispatch() -> void:
    entry: bb0
  
    bb0:
  
      return
  end
  
  public fn syliComplex_dispatch.main(%__unit.0:i64) -> void:
    entry: bb0
  
    bb0:
      gc_cycle
      %Sy_cir_var_0:obj{{card=2 [0:fn_ptr; 1:i64]} tag=0 unknow_cyclic} = object_create{size=2:i32}
      
      %Sy_oir_accum_fn_0:fn_ptr = addr_fn(__make_closure_accum.dispatch.67_ret_i64)
      obj_set(%Sy_cir_var_0:obj_ptr, 0:i32, %Sy_oir_accum_fn_0:fn_ptr):fn_ptr
      obj_set(%Sy_cir_var_0:obj_ptr, 1:i32, 1:i64):i64
      
      gc_cycle
      %Sy_cir_var_1:obj{{card=3 [0:fn_ptr; 1:i64; 2:obj_ptr]} tag=0 unknow_cyclic} = object_create{size=3:i32}
      
      %Sy_oir_accum_fn_1:fn_ptr = addr_fn(__partial_closure_accum.dispatch.clos0_arg2_ret_i64)
      obj_set(%Sy_cir_var_1:obj_ptr, 0:i32, %Sy_oir_accum_fn_1:fn_ptr):fn_ptr
      obj_set(%Sy_cir_var_1:obj_ptr, 1:i32, 5:i64):i64
      %Sy_oir_release_tmp_1:obj_ptr = @transfer obj_get(%Sy_cir_var_1:obj_ptr, 2:i32):obj_ptr
      release(%Sy_oir_release_tmp_1:obj_ptr)
      obj_set(%Sy_cir_var_1:obj_ptr, 2:i32, @share %Sy_cir_var_0:obj_ptr):obj_ptr
      
      %Sy_cir_var_2:i64 = #call_direct syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64 (@transfer %Sy_cir_var_1:obj_ptr, 10:i64, 20:i64)
      %Sy_cir_var_3:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_2:i64)
      gc_cycle
      %Sy_cir_var_4:obj{{card=3 [0:fn_ptr; 1:i64; 2:obj_ptr]} tag=0 unknow_cyclic} = object_create{size=3:i32}
      
      %Sy_oir_accum_fn_2:fn_ptr = addr_fn(__partial_closure_accum.dispatch.clos0_arg2_ret_i64)
      obj_set(%Sy_cir_var_4:obj_ptr, 0:i32, %Sy_oir_accum_fn_2:fn_ptr):fn_ptr
      obj_set(%Sy_cir_var_4:obj_ptr, 1:i32, 4:i64):i64
      %Sy_oir_release_tmp_2:obj_ptr = @transfer obj_get(%Sy_cir_var_4:obj_ptr, 2:i32):obj_ptr
      release(%Sy_oir_release_tmp_2:obj_ptr)
      obj_set(%Sy_cir_var_4:obj_ptr, 2:i32, @share %Sy_cir_var_0:obj_ptr):obj_ptr
      
      %Sy_cir_var_5:i64 = #call_direct syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64 (@transfer %Sy_cir_var_4:obj_ptr, 1.0f:f64, 2.0f:f64)
      %Sy_cir_var_6:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_5:i64)
      gc_cycle
      %Sy_cir_var_7:obj{{card=4 [0:fn_ptr; 1:i64; 2:obj_ptr; 3:i64]} tag=0 unknow_cyclic} = object_create{size=4:i32}
      
      %Sy_accum_fn_3:fn_ptr = addr_fn(__partial_closure_accum.dispatch.clos1_arg1_ret_i64)
      obj_set(%Sy_cir_var_7:obj_ptr, 0:i32, %Sy_accum_fn_3:fn_ptr):fn_ptr
      obj_set(%Sy_cir_var_7:obj_ptr, 1:i32, 2:i64):i64
      %Sy_oir_release_tmp_3:obj_ptr = @transfer obj_get(%Sy_cir_var_7:obj_ptr, 2:i32):obj_ptr
      release(%Sy_oir_release_tmp_3:obj_ptr)
      obj_set(%Sy_cir_var_7:obj_ptr, 2:i32, @share %Sy_cir_var_0:obj_ptr):obj_ptr
      obj_set(%Sy_cir_var_7:obj_ptr, 3:i32, 2:i64):i64
      
      %Sy_accum_ptr_4:fn_ptr = obj_get(%Sy_cir_var_7:obj_ptr, 0:i32):fn_ptr
      %Sy_cir_var_8:i64 = #call_direct_fn_ptr(%Sy_accum_ptr_4:fn_ptr)  (30:i64, @borrow %Sy_cir_var_7:obj_ptr, 1:i64)
      
      %Sy_cir_var_9:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_8:i64)
      %Sy_accum_ptr_5:fn_ptr = obj_get(%Sy_cir_var_7:obj_ptr, 0:i32):fn_ptr
      %Sy_apply_cast_6:i64 = cast(3.0f:f64 as i64)
      %Sy_cir_var_10:i64 = #call_direct_fn_ptr(%Sy_accum_ptr_5:fn_ptr)  (%Sy_apply_cast_6:i64, @transfer %Sy_cir_var_7:obj_ptr, 0:i64)
      
      %Sy_cir_var_11:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_10:i64)
      %Sy_cir_var_12:bool = cast(true:bool as bool)
      cond_br %Sy_cir_var_12:bool, bb1, bb2
  
    bb2:
      gc_cycle
      %Sy_cir_var_16:obj{{card=3 [0:fn_ptr; 1:i64; 2:obj_ptr]} tag=0 unknow_cyclic} = object_create{size=3:i32}
      
      %Sy_oir_accum_fn_7:fn_ptr = addr_fn(__partial_closure_accum.dispatch.clos0_arg2_ret_i64)
      obj_set(%Sy_cir_var_16:obj_ptr, 0:i32, %Sy_oir_accum_fn_7:fn_ptr):fn_ptr
      obj_set(%Sy_cir_var_16:obj_ptr, 1:i32, 1:i64):i64
      %Sy_oir_release_tmp_4:obj_ptr = @transfer obj_get(%Sy_cir_var_16:obj_ptr, 2:i32):obj_ptr
      release(%Sy_oir_release_tmp_4:obj_ptr)
      obj_set(%Sy_cir_var_16:obj_ptr, 2:i32, @own %Sy_cir_var_0:obj_ptr):obj_ptr
      
      %Sy_cir_var_17:i64 = #call_direct syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64 (@transfer %Sy_cir_var_16:obj_ptr, 1.0f:f64, 2.0f:f64)
      %Sy_cir_var_13:i64 = move(%Sy_cir_var_17:i64)
      goto bb3
  
    bb1:
      gc_cycle
      %Sy_cir_var_14:obj{{card=2 [0:fn_ptr; 1:obj_ptr]} tag=0 unknow_cyclic} = object_create{size=2:i32}
      
      %Sy_oir_accum_fn_8:fn_ptr = addr_fn(__partial_closure_accum.clos0_arg2_ret_i64)
      obj_set(%Sy_cir_var_14:obj_ptr, 0:i32, %Sy_oir_accum_fn_8:fn_ptr):fn_ptr
      %Sy_oir_release_tmp_5:obj_ptr = @transfer obj_get(%Sy_cir_var_14:obj_ptr, 1:i32):obj_ptr
      release(%Sy_oir_release_tmp_5:obj_ptr)
      obj_set(%Sy_cir_var_14:obj_ptr, 1:i32, @own %Sy_cir_var_0:obj_ptr):obj_ptr
      
      %Sy_cir_var_15:i64 = #call_direct syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64 (@transfer %Sy_cir_var_14:obj_ptr, 100:i64, 200:i64)
      %Sy_cir_var_13:i64 = move(%Sy_cir_var_15:i64)
      goto bb3
  
    bb3:
      %Sy_cir_var_18:void = #call_direct syliComplex_dispatch.syli_print_i64 (%Sy_cir_var_13:i64)
      return
  end
  
  public fn syliComplex_dispatch.apply__fn_f64_f64_i64__f64__f64_ret_i64(%f:obj_ptr, %x:f64, %y:f64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_accum_ptr_0:fn_ptr = obj_get(%f:obj_ptr, 0:i32):fn_ptr
      %Sy_apply_cast_1:i64 = cast(%x:f64 as i64)
      %Sy_apply_cast_2:i64 = cast(%y:f64 as i64)
      %Sy_cir_var_0:i64 = #call_direct_fn_ptr(%Sy_accum_ptr_0:fn_ptr)  (%Sy_apply_cast_1:i64, %Sy_apply_cast_2:i64, @transfer %f:obj_ptr, 0:i64)
      
      return %Sy_cir_var_0:i64
  end
  
  public fn syliComplex_dispatch.apply__fn_i64_i64_i64__i64__i64_ret_i64(%f:obj_ptr, %x:i64, %y:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_accum_ptr_0:fn_ptr = obj_get(%f:obj_ptr, 0:i32):fn_ptr
      %Sy_cir_var_0:i64 = #call_direct_fn_ptr(%Sy_accum_ptr_0:fn_ptr)  (%x:i64, %y:i64, @transfer %f:obj_ptr, 0:i64)
      
      return %Sy_cir_var_0:i64
  end
  
  public fn syliComplex_dispatch.add__i64__i64__f64_ret_i64(%x:i64, %y:i64, %z:f64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  public fn syliComplex_dispatch.add__i64__i64__i64_ret_i64(%x:i64, %y:i64, %z:i64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  public fn syliComplex_dispatch.add__i64__f64__f64_ret_i64(%x:i64, %y:f64, %z:f64) -> i64:
    entry: bb0
  
    bb0:
  
      return %x:i64
  end
  
  private fn __make_closure_accum.dispatch.67_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_clos:obj_ptr, %Sy_oir_dp_id:i64) -> i64:
    entry: bb-1
  
    bb-1:
      %Sy_oir_imm0:i64 = obj_get(%Sy_oir_clos:obj_ptr, 1:i64):i64
      release(%Sy_oir_clos:obj_ptr)
      switch %Sy_oir_dp_id:i64 [1: bb1, 0: bb0, 2: bb2, 3: bb3, 4: bb4, 5: bb5]
  
    bb1:
      %Sy_oir_case_result1:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_f64_f64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result1:i64
  
    bb0:
      %Sy_oir_case_result0:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_i64_i64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result0:i64
  
    bb2:
      %Sy_oir_case_result2:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_i64_f64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result2:i64
  
    bb3:
      %Sy_oir_case_result3:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_i64_i64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result3:i64
  
    bb4:
      %Sy_oir_case_result4:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_f64_f64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result4:i64
  
    bb5:
      %Sy_oir_case_result5:i64 = #call_direct __wrapper.syliComplex_dispatch.add.i64_i64_i64_ret_i64 (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, %Sy_oir_x1:i64)
      return %Sy_oir_case_result5:i64
  end
  
  private fn __partial_closure_accum.clos0_arg2_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_clos:obj_ptr, %Sy_oir_dp_id:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_p_clos:obj_ptr = @share obj_get(%Sy_oir_clos:obj_ptr, 1:i64):obj_ptr
      release(%Sy_oir_clos:obj_ptr)
      %Sy_oir_p_accum:fn_ptr = obj_get(%Sy_oir_p_clos:obj_ptr, 0:i64):fn_ptr
      %Sy_oir_rst:i64 = #call_direct_fn_ptr(%Sy_oir_p_accum:fn_ptr)  (%Sy_oir_x0:i64, %Sy_oir_x1:i64, @transfer %Sy_oir_p_clos:obj_ptr, %Sy_oir_dp_id:i64)
      return %Sy_oir_rst:i64
  end
  
  private fn __partial_closure_accum.dispatch.clos0_arg2_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_clos:obj_ptr, %Sy_oir_dp_id:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_dp_clos:i64 = obj_get(%Sy_oir_clos:obj_ptr, 1:i64):i64
      %Sy_oir_accum_dp_id:i64 = %Sy_oir_dp_id:i64 + %Sy_oir_dp_clos:i64
      %Sy_oir_p_clos:obj_ptr = @share obj_get(%Sy_oir_clos:obj_ptr, 2:i64):obj_ptr
      release(%Sy_oir_clos:obj_ptr)
      %Sy_oir_p_accum:fn_ptr = obj_get(%Sy_oir_p_clos:obj_ptr, 0:i64):fn_ptr
      %Sy_oir_rst:i64 = #call_direct_fn_ptr(%Sy_oir_p_accum:fn_ptr)  (%Sy_oir_x0:i64, %Sy_oir_x1:i64, @transfer %Sy_oir_p_clos:obj_ptr, %Sy_oir_accum_dp_id:i64)
      return %Sy_oir_rst:i64
  end
  
  private fn __partial_closure_accum.dispatch.clos1_arg1_ret_i64(%Sy_oir_x0:i64, %Sy_oir_clos:obj_ptr, %Sy_oir_dp_id:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_dp_clos:i64 = obj_get(%Sy_oir_clos:obj_ptr, 1:i64):i64
      %Sy_oir_accum_dp_id:i64 = %Sy_oir_dp_id:i64 + %Sy_oir_dp_clos:i64
      %Sy_oir_p_clos:obj_ptr = @share obj_get(%Sy_oir_clos:obj_ptr, 2:i64):obj_ptr
      %Sy_oir_p_accum:fn_ptr = obj_get(%Sy_oir_p_clos:obj_ptr, 0:i64):fn_ptr
      %Sy_oir_imm0:i64 = obj_get(%Sy_oir_clos:obj_ptr, 3:i64):i64
      release(%Sy_oir_clos:obj_ptr)
      %Sy_oir_rst:i64 = #call_direct_fn_ptr(%Sy_oir_p_accum:fn_ptr)  (%Sy_oir_imm0:i64, %Sy_oir_x0:i64, @transfer %Sy_oir_p_clos:obj_ptr, %Sy_oir_accum_dp_id:i64)
      return %Sy_oir_rst:i64
  end
  
  private fn __wrapper.syliComplex_dispatch.add.i64_f64_f64_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_x2:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_s1:f64 = cast(%Sy_oir_x1:i64 as f64)
      %Sy_oir_s2:f64 = cast(%Sy_oir_x2:i64 as f64)
      %Sy_oir_rst:i64 = #call_direct syliComplex_dispatch.add__i64__f64__f64_ret_i64 (%Sy_oir_x0:i64, %Sy_oir_s1:f64, %Sy_oir_s2:f64)
      return %Sy_oir_rst:i64
  end
  
  private fn __wrapper.syliComplex_dispatch.add.i64_i64_f64_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_x2:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_s2:f64 = cast(%Sy_oir_x2:i64 as f64)
      %Sy_oir_rst:i64 = #call_direct syliComplex_dispatch.add__i64__i64__f64_ret_i64 (%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_s2:f64)
      return %Sy_oir_rst:i64
  end
  
  private fn __wrapper.syliComplex_dispatch.add.i64_i64_i64_ret_i64(%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_x2:i64) -> i64:
    entry: bb0
  
    bb0:
      %Sy_oir_rst:i64 = #call_direct syliComplex_dispatch.add__i64__i64__i64_ret_i64 (%Sy_oir_x0:i64, %Sy_oir_x1:i64, %Sy_oir_x2:i64)
      return %Sy_oir_rst:i64
  end
  
  end

